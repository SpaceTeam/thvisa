...
class VNA(ff.fieldfox):
    ...
    def __init__(self, instrname = instrnamedef, qdelay, myprint = myprintdef):
    ...
    def do_sweeps(self, continous="off"):
        ...
        self.do_command("INIT:CONT "+str(continous)+";*OPC?")
        ...
        for i in range(self.avgs): # have to manually trigger each run for the averaging..
            ret = self.do_query_string("INIT:IMM;*OPC?")
    ...
    def get_trace_mag(self, trace=1,save_trace=0):
        ...
        self.do_command("CALC:PAR"+trace+":SEL")
    ...
    def collect_traces_mag(self):
        for i in [1,2,3,4]:
            self.traces.append(get_trace_mag())


    def collect_traces(self):
        for i in [1,2,3,4]:
            self.traces.append(get_trace())
